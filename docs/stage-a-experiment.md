...
- **B 路：MCTS-400（带 value）vs 屏蔽 value 的 MCTS = 24胜7和1负，+314.4 Elo（CI [+210, +506]，32 局）**——value 头在 400 sims 下贡献约 +314 Elo，是真实的搜索增益来源之一（policy 先验承担其余部分）。

**最终归因**：适配链路健康（③全绿）＋搜索增益巨大（A 路 +720）＋ value 头有效（B 路 +314）⇒ **−880 Elo 的差距是网络本身的对抗棋力不足**（纯 policy 约 −1600 水平，搜索拉到 −880，距 champion 仍远）。这是评审决策表中"接口一致、搜索有增益、但整体仍远弱于 champion ⇒ 进入小规模 RL 验证"的分支。评审转发的另一份评价中的假设 1（value 弱致搜索零增益）被 A 路直接证伪；假设 2a（WDL 顺序）此前已被代码核实排除；假设 2b（超时）经审计无实例。

补充工程记录：归因对照期间发现 held-out 评估进程结束后未退出（dataloader worker 未回收，白占 7GB 显存 2 小时），已清理；infer server 峰值保留 8.6GB（缓存分配器保留历史最大激活块，非泄露），两者均记入 Stage B 改进清单。
