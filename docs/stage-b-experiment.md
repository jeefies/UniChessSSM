...
> **⚠️ 本节结论已于 2026-09-19 被推翻（commit 784dc64，详见 `design-deviations.md` §7）。**
> 当时的"交叉核对"脚本与生成器犯了同一个错：都用**严格**判定 `is_repetition(3)` /
> `is_fifty_moves()`，而对局循环用 `is_game_over(claim_draw=True)` 退出——后者含"下一着
> 可申和"，早一 ply。于是核对自洽地得出"不匹配 0 局、五十步 0、三次重复 0"，
> 而真相是**所有规则申和局都被误记为 300 ply 封顶截断**。
> 按 `outcome(claim_draw=True)` 重新裁决全部分片后的真实分布见 §5.1 修正行：
> 三次重复是仅次于将杀的第二大终局原因（gen2k 38.0%），真实封顶率仅 15.9%。
> 下表保留作错误记录，**不得引用**。
...
> **⚠️ 上表数字已作废（2026-09-19 复测）。** 它与 §11.3 记录的 H(π′)≈1.94 直接矛盾，
> 该矛盾此前挂在"差异来源待查"。在当前 `stage_b_gen2k` 分片上重新统计：
>
> | 指标 | 值 |
> |---|---|
> | H(π′) 均值 | **1.2644** |
> | H(π′) 中位数 | **1.4447** |
> | 零熵（硬目标）占比 | **8.1%** |
> | max_prob > 0.999 占比 | **8.5%** |
>
> 即 π′ **并非** 75% 硬目标，而是绝大多数为软分布；上表的 0.0605 不描述任何现存数据
> （疑为早期 n_sims=32 + 未修 `_resolve_move` 时期的统计，或统计脚本自身有误）。
> 结论相应修正：**soft CE 不可退化为 hard CE**，`policy_soft_loss` 的数值安全路径是
> 必需的而非冗余的（现有实现已正确使用，通过 A#6+ 安全测试验证）。
> σ(completedQ) 确有锐化作用，但强度远未到 one-hot。
...
> **⚠️ 上表"封顶率"与"五十步/重复"行已作废（2026-09-19，commit 784dc64）。** 生成器
> 终局分类口径错误，把规则申和局记成了封顶截断。全部分片按 `outcome(claim_draw=True)`
> 重放重算（`tools/repair_v3_meta.py`，已就地修复 meta）后的真实分布：

| 终止原因 | 冒烟 1,000 局 | 验证集 256 局 | **首轮 2,000 局** | Round 2 2,500 局 |
|---|---|---|---|---|
| 将杀 | （未重算，该目录已隔离） | 102（39.8%） | **664（33.2%）** | 858（34.3%） |
| 逼和 | — | 14（5.5%） | **144（7.2%）** | 176（7.0%） |
| 子力不足 | — | 10（3.9%） | **108（5.4%）** | 140（5.6%） |
| **三次重复** | — | **101（39.5%）** | **760（38.0%）** | **898（35.9%）** |
| 五十步 | — | 0 | **6（0.3%）** | 8（0.3%） |
| **封顶截断** | — | **29（11.3%）** | **318（15.9%）** | **420（16.8%）** |
| meta 修正局数 | — | 101 | 766 | 906 |
| **`result`（z 标签）修正局数** | — | **0** | **0** | **0** |

> z 标签零修正 ⇒ 动作序列 / π′ / z 全部正确，**无需重跑生成**。
> 连带影响：`mlh_valid_mask` 此前错误剔除 ~54% 对局，修复后 gen2k 有效局
> **46% → 84.1%**、round2 **→ 83.2%**，moves-left 头的有效样本量翻倍。
> 真实封顶率 ~16% 仍高于 §1.2 设定的 <10% 期望，但远低于此前记录的 54%，
> 性质从"管道异常"降级为"策略偏和"。
...
> **⚠️ §7.1/§7.2 的 arena 数字产自含开局历史 bug 的旧 arena（2026-09-19 修复，
> commit 784dc64）：开局着法 push 在 encode 之前，B₀ 从未入 R。最新可信结果见 §13.3b。**
...
> **⚠️ 8.1–8.4 基于错误数据，已于 2026-09-19 整节作废（commit 784dc64）。**
> §8.2 的核心推理以"五十步=0、三次重复=0"为由排除了「重复/五十步绕行导致封顶」，
> 而这两个 0 恰是终局分类 bug 的产物。真相相反：**三次重复是第二大终局原因
> （gen2k 38.0%）**，"绕行"非但未被排除，还是主导现象；真实封顶率仅 **15.9%**。
> 修正后的事实与结论见 §8.5；以下四小节保留作错误记录，**不得引用**。
...
### 8.5 修正后的事实与结论（2026-09-19）

| 指标 | 验证集 256 局 | **首轮 2,000 局** | Round 2 2,500 局 |
|---|---|---|---|
| 将杀率 | 39.8% | **33.2%** | 34.3% |
| **三次重复** | **39.5%** | **38.0%** | **35.9%** |
| 逼和率 | 5.5% | **7.2%** | 7.0% |
| 子力不足 | 3.9% | **5.4%** | 5.6% |
| 五十步 | 0 | **0.3%** | 0.3% |
| **真实封顶率（局级）** | **11.3%** | **15.9%** | **16.8%** |
| **真实封顶率（位置级 ply）** | **17.6%** | **23.7%** | **25.2%** |

> 位置级占比高于局级（截断局偏长，每局贡献满 300 ply），但与此前记录的 **80.7%** 完全不同。
> 两个口径都应记录：训练看到的是位置，数据健康门槛（§1.2 的 20% 调查线）按局级计。

修正后的判断：

1. **"封顶危机"大部分是记账错误。** 真实封顶率 11–17%，略高于 §1.2 的 <10% 期望，
   但不构成管道异常，不触发">20% 须停下来调查"的条款。
2. **真正的现象是"重复和"**：约 36–40% 的对局以三次重复告终。旧 §8.2 对
   「重复/五十步绕行」的排除因此不成立。
   > **但"重复和多 ⇒ 终局兑现能力弱"目前只是假设，未被证实。** 三次重复只描述**结束方式**，
   > 不说明进入重复前是哪一方占优、是否存在可兑现的胜势：均势下主动求和、劣势下成功逼和、
   > 优势下错失胜机，都会产生三次重复。要确认需补充"进入重复前若干 ply 的评估分布 +
   > 子力/局面优势"分析。**在此之前不据此修改和棋奖励或搜索目标。**
3. **训练标签影响大幅缓解**：来自"人工和棋"的 value 监督从约 73–81% 的 ply 降到实际封顶
   局对应的比例；且三次重复是**规则真和棋**，其 z=和是真值而非工程约定。
   `mlh_valid_mask` 的有效局从 46% 回升到 84%。
4. **§8.4 建议相应调整**：「封顶局 value 降权」的动机已基本消失（封顶局占比小且标签多为真和）；
   「调查封顶局末 30 ply」应改为**调查重复和对局的中残局兑现**；「cap 升至 400」继续搁置。
...
### 13.3b A/B 再次重跑（修复开局历史 bug 后，2026-09-19，commit 784dc64）

上表 28/4 因 **arena 开局库 bug** 再次失效：开局着法写成 `push_san → _advance`，
导致初始局面 B₀ 从未进入 R、最后一个开局局面被重复步进两次（occurrence 也多记一次）。
两侧模型都是从一个**错误的历史状态**开始搜索的。改为 encode-before-move 后重跑
（`runs/arena_aa_round4` / `runs/arena_ab_round4`）：

| 指标 | 计分 bug 时 | 开局 bug 时（28/4） | **本次（可信）** |
|---|---|---|---|
| W/D/L（A=Stage A, B=Round 2） | 12/0/20 | 28/0/4 | **22/2/8** |
| A 得分率 | 37.5% | 87.5% | **71.9%** |
| 终止原因 | 32 checkmate | 32 checkmate | **30 checkmate + 2 子力不足** |
| 用时 | 218s | 218s | **1,762s** |
| anomalies | 0 | 0 | **0** |
| A/A 对称性 | — | 2/2（50%） | **2/2（50%），4/4 checkmate** |

> **修正后的结论**：
> 1. Stage A 仍明显强于 Round 2，但优势是 **71.9% 而非 87.5%**——此前的 28/4 被错误的
>    开局历史放大了。换代门槛 55%（400 局）的判断方向不变：**Round 2 不应晋级**。
> 2. 用时从 218s 涨到 1,762s（8×）：修正历史后对局不再一边倒地速杀，平均局长显著变长，
>    且出现了非将杀终局（2 局子力不足）。这本身是"历史正确后棋力表现更合理"的旁证。
> 3. A/A 依旧 50% 对称、0 anomaly，arena 自身无偏。
> 4. 32 局的置信区间仍很宽（71.9% ±~16pp），不足以做换代决策；正式换代须按 §2.8 C 组
>    跑 400 局。
...
## 14. 第三轮修复与验证（2026-09-19，commit 154d673 起）

详见 `design-deviations.md` §8。本节只记实验数字。

### 14.1 数据可用性定级（新增 `manifest["provenance"]`）

四个数据集**全部生成于 `f1146ac`（adapter 修复，2026-09-19 00:04）之前**，
当时 q 由**原始 WDL logits** 直接相减、Elo 未标准化、终局叶子用网络估值——三项都进
completedQ → σ → π′。故：

| 数据集 | actions | result z | termination meta | **π′** |
|---|---|---|---|---|
| `stage_b_smoke` | 已隔离（504/1000 动作解析错误） | — | — | `legacy_teacher` |
| `stage_b_val64` | ✅ | ✅ | ✅ 已修复 | **`legacy_teacher`** |
| `stage_b_gen2k` | ✅ | ✅ | ✅ 已修复 | **`legacy_teacher`** |
| `stage_b_gen_round2` | ✅ | ✅ | ✅ 已修复 | **`legacy_teacher`** |

**`result 修正数 = 0` 不能推出 π′ 正确**——它只说明 z 在重新裁决前后一致。

### 14.2 meta 修复影响范围核验

`*.actions.bin` / `*.pipol.bin` / `*.pipol.offsets.bin` 的 mtime 仍为生成日（09-17/09-18），
仅 `*.meta.npz` 为 09-19 → **只改了元数据**。
疏漏：未保留原始 meta 副本；旧值可由严格判定链重放确定性重建。

### 14.3 修复后训练冒烟 4/4 PASS（`tools/verify_meta_repair_smoke.py`）

固定种子、`dropout=0`、无 autocast，8 局取样（其中 5 局被恢复）。

| 检查 | 结果 |
|---|---|
| 1 旧/新 meta 前向分量 | policy 4.52785015 / value 2.37362790 / recon 0.04448702 / dyn 0.12660037 **逐位一致** |
| | `loss_mlh` 50.353260 → **56.997185**；有效位 **423 → 1,230** |
| 2 mlh_valid 全 False | `loss_mlh = 0.000e+00`，finite，无除零 |
| 3 恢复样本 moves_left | n_plies = 115/138/145/206/203，**均 <300** |
| 4 4 次真实更新 | loss 12.2343→8.1410；裁剪前梯度范数 36.44→12.17；全部有限；checkpoint 往返 OK |

> **mlh 损失不降反升（50.35→57.00）**：监督覆盖扩大 ≠ 梯度按局数比例放大——
> 按有效位置平均时分母同时改变。**维持 `w_mlh=0.1` 不动**，先观察真实贡献。
> **修复 meta 不修复已完成的历史更新**：Round 1/2 仍是旧有效位下训练的检查点，
> 不得重新标注为"使用修复后监督训练的模型"。

### 14.4 Q 归一化规格变更验证

新增 `gumbel.qtransform_completed`（逐节点 completed-Q 量程），根评分/非根选择/π′ 导出共用。
新增 4 项回归测试，全量单测 **74/74 PASS**。按审查意见，本项用固定局面数值对照与既有
算法测试收口，**未跑 A/B**。

### 14.5 修复后搜索首批数据（`runs/stage_b_gen_fix500`，500 局）

三处修复全部生效后的首批生成：adapter（`f1146ac`：softmax Q / 标准化 Elo / 终局真值）
+ 终局裁决（`784dc64`）+ 逐节点 Q 归一化（`154d673`）。
配置同旧批：Stage A champion、Gumbel n=64/m₀=16、g=1.0、4 workers × 24 并发、gen_id=3。
`provenance.teacher_status = current_teacher`（π′ **可用**）。

| 指标 | LEGACY `gen2k`（旧教师） | **NEW `fix500`（修复后）** |
|---|---|---|
| 局数 | 2,000 | 500 |
| 平均局长 | 201.5 ply（中位 210） | **88.4 ply（中位 78）** |
| **将杀** | 33.2% | **94.4%** |
| **三次重复** | 38.0% | **0.2%** |
| 五十步 | 0.3% | 2.0% |
| 子力不足 | 5.4% | 3.4% |
| 逼和 | 7.2% | **0%** |
| **封顶截断** | 15.9% | **0%** |
| games/s | 0.134 | **0.427**（3.2×） |
| plies/s | 27.0 | 37.8 |
| avg_sims/ply | 64.0 | 64.0（budget_violations=0） |
| avg 树深 | — | 7.07 |
| **H(π′) 均值** | **1.2644** | **0.1889** |
| H(π′) 中位 | 1.4447 | 0.0016 |
| 零熵占比 | 8.1% | **31.1%** |
| max_prob > 0.999 | 8.5% | **58.0%** |
| 平均合法着 | 26.5 | 25.4 |

完整性：500/500 局重放后 `result` 与 `classify_final_board` 一致（0 不符），
pipol 概率和异常 0，预算守恒 0 违规。

#### 两个方向相反的结果

**① 对局质量大幅改善。** 将杀率 33.2% → 94.4%，三次重复 38.0% → **0.2%**，封顶 15.9% → **0%**，
局长减半。**这强烈提示旧数据中"重复和主导"是坏搜索的产物，而不是模型残局兑现能力的体现**
——§8.5/§8.7 中"兑现能力弱"的假设因此进一步削弱，不应再作为改进方向的依据。

**② π′ 坍缩向 one-hot。** 熵 1.2644 → 0.1889，58% 的目标 max_prob > 0.999。
原因是逐节点归一化把分母从全树量程换成了本节点量程（常常只有 ~0.2），
而 σ 展幅 = (c_visit + maxN)·c_scale·1.0 ≥ 51，远压过 ℓ 的量级。
**后果：soft CE 事实上退化为 hard CE**，§4.3 修正后的结论（"soft CE 不可退化"）
对**新数据**不再成立——软目标带来的信息量基本丢失。

#### 归因未分离（记录在案）

新旧两批之间同时变了**两组**东西：adapter 修复与 Q 归一化定义。
按 σ 与 ℓ 的量级推导，**熵坍缩几乎可以确定来自逐节点归一化**；
而将杀率提升更可能来自 adapter 修复（终局真值 + 校准后的 q）。
**但本批数据不能证明这一拆分。** 要分离需一次消融：保持 adapter 修复、
仅将归一化切回全树量程，生成同规模小批对照。

#### 未动的超参

按既定纪律，本轮**不调整 `c_scale`**，也不因熵坍缩回退归一化定义——
先记录现象。`c_scale` 是与归一化定义**不同**的一个变量，若要压低 σ 展幅
（例如 `c_scale` 由 1.0 降至 ~0.1，使 σ 展幅回到 ℓ 的量级），应作为独立变更单独验证。
