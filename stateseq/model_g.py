...
class DynamicsG(nn.Module):
    ...
    def __init__(self, d_model: int = D_MODEL, n_actions: int = NUM_ACTIONS):
        ...
        self.fc1 = nn.Linear(2 * d_model, 2 * d_model)  # 1024→1024，残差（同形堆叠统一形式）
        self.fc2 = nn.Linear(2 * d_model, d_model)       # 1024→512
    ...
    def forward(self, h_prev: torch.Tensor, action: torch.Tensor) -> torch.Tensor:
        ...
        z = torch.cat([self.norm(h_prev), self.emb_a(action.long())], dim=-1)  # (..., 1024)
        h = z + F.gelu(self.fc1(z))                      # 首层残差（1024 同维）
        u = self.fc2(F.gelu(h))                          # (..., 512)
