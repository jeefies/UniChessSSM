...
def main():
    ...
    for gi in range(min(n_games, 50)):
        ...
        for ply in range(min(len(actions), 50)):
            ...
            search_records.append({
                ...
                "delta_sigma": round(float(sig_search.max() - sig_search.min()), 6),
                "delta_logit": round(float(logits_masked.max() - logits_masked.min()), 6),
                "delta_sigma_vs_delta_logit_ratio": round(
                    (sig_search.max() - sig_search.min()) / max(abs(logits_masked.max() - logits_masked.min()), 1e-10), 4),
                "q_min": round(qmin_search, 6),
                "q_max": round(qmax_search, 6),
                "q_normalized_range": round(float(cq_norm_search.max() - cq_norm_search.min()), 6),
    ...
    summary["search_trace"] = {
        ...
        "delta_metrics": {
            "delta_sigma_mean": round(float(np.mean([r["delta_sigma"] for r in search_records])), 4),
            "delta_sigma_max": round(float(np.max([r["delta_sigma"] for r in search_records])), 4),
            "delta_logit_mean": round(float(np.mean([r["delta_logit"] for r in search_records])), 4),
            "delta_logit_max": round(float(np.max([r["delta_logit"] for r in search_records])), 4),
            "delta_ratio_mean": round(float(np.mean([r["delta_sigma_vs_delta_logit_ratio"] for r in search_records])), 4),
            "delta_ratio_max": round(float(np.max([r["delta_sigma_vs_delta_logit_ratio"] for r in search_records])), 4),
        },
        "q_normalized_range": {
            "mean": round(float(np.mean([r["q_normalized_range"] for r in search_records])), 6),
            "max": round(float(np.max([r["q_normalized_range"] for r in search_records])), 6),
        },
