...
批量对弈配置示例（``python -m Kit match``）::

    {"factory": "SSM.kit:make_player_factory",
     "root": "/home/jeefy/UniChess",
     "kwargs": {"checkpoint": "SSM/runs/stage_b/gen3.pt", "simulations": 256}}
...
from Kit.api import EvalRequest, GameStart, MoveDecision, NodeEval, SearchBudget, immediate
from Kit.search.gumbel import Gumbel, GumbelConfig, softmax
...
# ---------------------------------------------------------------- 终局裁决（v3 meta 口径）
# 自对弈的终止原因编码。**顺序即 v3 meta.termination_reason 的取值**，改了箱底数据就作废，
# 新增原因只能往后追加。
...
def get_termination_reason_from_board(board: chess.Board) -> tuple[str, bool]:
    """从分片读取时重构终止原因（不依赖 max_plies，用实际局面判断）。"""
    outcome = board.outcome(claim_draw=True)
    if outcome is None:
...
def get_terminal_q(board: chess.Board) -> float:
    ...
    if outcome.winner is None:
        return 0.0
    # outcome.winner 是胜方颜色；当前行棋方视角：board.turn == outcome.winner → +1，否则 −1
...
def make_evaluator(checkpoint, device: str = "cuda", engine: str = "fast", *,
    ...
    if engine == "server":
        ...
        from .infer.gpu_server import remote_evaluator
    ...
    if engine == "fast":
        from .infer.fast_eval import SsmFastEvaluator
